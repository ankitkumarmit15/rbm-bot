...
# ─────────────────────────────────────────────────────────────
# ─────────────────────────────────────────────────────────────
# Netcracker Enterprise Theme CSS
# ─────────────────────────────────────────────────────────────
st.markdown("""
<style>
/* ═══════════════════════════════════════════════════════════
   NETCRACKER ENTERPRISE THEME
   Navy:        #002060  — primary brand (Netcracker)
   Navy-dark:   #001040  — hero, sidebar header accent
   Navy-mid:    #003182  — hover states
   Electric:    #00A3E0  — accent / highlight
   Electric-lt: #E5F6FD  — tinted surfaces
   Surface:     #FFFFFF  — cards, sidebar
   BG:          #F0F4F9  — page background
   Border:      #D0DCEE  — dividers
   Text:        #12253E  — primary text
   Muted:       #5A6680  — secondary text
   Success:     #00875A  — positive states
   Warning:     #D97706  — caution states
   Danger:      #C53030  — error states
═══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════
   SIDEBAR — clean white with navy accent rail
══════════════════════════════════════════════ */
[data-testid="stSidebar"],
[data-testid="stSidebar"] > div {
    background: #FFFFFF !important;
    border-right: 1px solid #D0DCEE !important;
    box-shadow: 3px 0 16px rgba(0,32,96,.07) !important;
}
[data-testid="stSidebar"] label,
[data-testid="stSidebar"] p,
[data-testid="stSidebar"] span,
[data-testid="stSidebar"] .stMarkdown p { color: #12253E !important; }
[data-testid="stSidebar"] .stCaption,
[data-testid="stSidebar"] small         { color: #5A6680 !important; }
...
/* ══════════════════════════════════════════════
   BUTTONS — Netcracker navy primary
══════════════════════════════════════════════ */
.stButton > button {
...
.stButton > button[kind="primary"] {
    ...
    box-shadow: 0 2px 8px rgba(0,32,96,.28) !important;
}
.stButton > button[kind="primary"]:hover {
    background: #003182 !important;
    box-shadow: 0 4px 16px rgba(0,32,96,.38) !important;
}
...
.stButton > button[kind="tertiary"] {
    ...
    border: 1px solid #D0DCEE !important;
    font-size: 12px !important;
}

/* ══════════════════════════════════════════════
   DOWNLOAD BUTTON — electric teal
══════════════════════════════════════════════ */
.stDownloadButton > button {
    ...
    font-weight: 600 !important;
    font-family: 'Inter', sans-serif !important;
    box-shadow: 0 2px 8px rgba(0,135,90,.22) !important;
...
/* ══════════════════════════════════════════════
   FILE UPLOADER — neutral professional look
══════════════════════════════════════════════ */
...
[data-testid="stFileUploader"] > div {
    background: #F5F8FC !important;
    border: 2px dashed #A8BEDD !important;
    border-radius: 8px !important;
    color: #12253E !important;
}
...
[data-testid="stFileUploaderDropzoneInstructions"] small { color: #002060 !important; }
[data-testid="stFileUploaderDropzoneInstructions"] svg   { fill: #00A3E0 !important; }
...
.stTextInput > div > div {
    background: #FFFFFF !important;
    border: 1.5px solid #C8D8ED !important;
    border-radius: 6px !important;
    color: #12253E !important;
    font-family: 'Inter', sans-serif !important;
}
...
.stTextInput > div > div:focus-within {
    border-color: #00A3E0 !important;
    box-shadow: 0 0 0 3px rgba(0,163,224,.15) !important;
}
...
[data-testid="stExpander"] {
    ...
    border: 1px solid #D0DCEE !important;
    border-radius: 8px !important;
    box-shadow: 0 1px 4px rgba(0,32,96,.05) !important;
    margin: 5px 0 !important;
    overflow: hidden !important;
}
[data-testid="stExpander"]:hover {
    box-shadow: 0 3px 12px rgba(0,32,96,.10) !important;
    border-color: #A8BEDD !important;
}
...
div.stAlert > div[data-testid="stMarkdownContainer"] { color: #12253E !important; }
[data-testid="stAlert"][data-type="info"]    { background: #EBF5FB !important; border-color: #00A3E0 !important; }
[data-testid="stAlert"][data-type="warning"] { background: #FFF8EC !important; border-color: #D97706 !important; }
[data-testid="stAlert"][data-type="success"] { background: #EAFAF3 !important; border-color: #00875A !important; }
[data-testid="stAlert"][data-type="error"]   { background: #FEF2F2 !important; border-color: #C53030 !important; }

/* ══════════════════════════════════════════════
   CODE BLOCK / EXECUTION LOG
══════════════════════════════════════════════ */
[data-testid="stCode"] > div, pre, code {
    background: #F0F4F9 !important;
    border: 1px solid #D0DCEE !important;
    border-radius: 6px !important;
    color: #12253E !important;
...
[data-testid="stChatMessage"] {
    ...
    box-shadow: 0 1px 6px rgba(0,32,96,.06) !important;
}
...
[data-testid="stDataEditor"] {
    ...
    overflow: hidden !important;
    border: 1px solid #D0DCEE !important;
    box-shadow: 0 2px 8px rgba(0,32,96,.06) !important;
}

/* ══════════════════════════════════════════════
   MISC CONTROLS
══════════════════════════════════════════════ */
.stCheckbox label span { color: #12253E !important; font-weight: 500 !important; }
hr { border-color: #D0DCEE !important; margin: 10px 0 !important; }
[data-testid="stSpinner"] p { color: #002060 !important; }
...
.qa-brand-name {
    font-size: 13px; font-weight: 700; color: #002060; line-height: 1.25;
    font-family: 'Inter', sans-serif; letter-spacing: -0.01em;
}
.qa-brand-sub {
    font-size: 10px; color: #8095B4; text-transform: uppercase;
    letter-spacing: .8px; margin-top: 1px; font-weight: 500;
}
...
.qa-sb-hdr-icon { font-size: 12px; }
.qa-sb-hdr-text {
    font-size: 10.5px; font-weight: 700; color: #002060;
...
.qa-chip-ok   { background: #EAFAF3; color: #006644; border-color: #A3D9C0; }
.qa-chip-warn { background: #FFF8EC; color: #92400E; border-color: #F9C97A; }
.qa-chip-err  { background: #FEF2F2; color: #9B1C1C; border-color: #F3A8A8; }
.qa-chip-info { background: #EBF5FB; color: #003A6B; border-color: #7EC8E3; }

/* ══════════════════════════════════════════════
   ANIMATIONS
══════════════════════════════════════════════ */
@keyframes qa-pulse    { 0%,100%{opacity:1} 50%{opacity:.5} }
@keyframes qa-slideUp  { from{opacity:0;transform:translateY(12px)} to{opacity:1;transform:translateY(0)} }
...
.qa-pipeline {
    ...
    animation: qa-slideUp 0.45s ease;
}
.qa-badge {
    display: inline-flex; align-items: center; gap: 5px;
...
.qa-badge-idle   { background: rgba(255,255,255,.12); color: rgba(255,255,255,.80);
                   border-color: rgba(255,255,255,.25); }
.qa-badge-active { background: #FFF4CC; color: #7A4800; border-color: #F5C842;
                   animation: qa-pulse 1.1s infinite; }
.qa-badge-done   { background: #D6F5E8; color: #005C38; border-color: #6FD4A8; }
.qa-arrow        { color: rgba(255,255,255,.35); font-size: 16px; }

/* ══════════════════════════════════════════════
   METRIC CARDS — enterprise data dashboard style
══════════════════════════════════════════════ */
.qa-metrics {
    display: grid;
...
.qa-metric {
    ...
    box-shadow: 0 1px 4px rgba(0,32,96,.06);
    transition: box-shadow 0.2s ease, border-top-color 0.2s ease;
    position: relative;
}
.qa-metric:hover {
    box-shadow: 0 4px 14px rgba(0,32,96,.12);
    border-top-color: #00A3E0;
}
.qa-metric-val   { font-size: 1.85rem; font-weight: 800; color: #002060; line-height: 1.1;
                   font-family: 'Inter', sans-serif; letter-spacing: -0.03em; }
.qa-metric-label { font-size: 10px; color: #7A90B0; font-weight: 600;
...
.qa-kb-doc {
    ...
    border-radius: 6px;
    padding: 7px 11px;
    font-size: 12.5px; color: #12253E;
    margin: 3px 0;
    animation: qa-fadeIn 0.25s ease;
    font-family: 'Inter', sans-serif;
}
.qa-kb-small { font-size: 11px; color: #7A90B0; margin-top: 2px; }

/* ══════════════════════════════════════════════
   SECTION HEADER BANNERS (results area)
══════════════════════════════════════════════ */
.qa-section {
    background: #EBF0F8;
    border-radius: 8px;
    ...
    animation: qa-fadeIn 0.35s ease;
}
.qa-section h3 { margin: 0; color: #002060; font-size: 14px; font-weight: 700; }
.qa-section p  { margin: 3px 0 0; color: #5A6680; font-size: 12.5px; }
</style>
...
def _bars_html(counts: dict, colors: dict, default_color: str = "#002060") -> str:
    ...
    for label, cnt in sorted(counts.items(), key=lambda x: -x[1]):
        ...
        rows += (
            ...
            "<div style='width:100px;font-size:11px;color:#5A6680;text-align:right;"
            "white-space:nowrap;overflow:hidden;text-overflow:ellipsis;'>" + label + "</div>"
            "<div style='flex:1;background:#E8EFF8;border-radius:3px;height:14px;overflow:hidden;'>"
            "<div style='background:" + color + ";width:" + str(pct) + "%;height:100%;border-radius:3px;'></div>"
            "</div>"
            "<div style='width:50px;font-size:11px;font-weight:600;color:#12253E;white-space:nowrap;'>"
            + str(cnt) + " <span style='color:#8095B4;font-weight:400;'>(" + str(pct) + "%)</span></div>"
            "</div>"
        )
...
st.markdown("""
<div style="
    background: linear-gradient(135deg, #001040 0%, #002060 55%, #003182 100%);
    border-radius: 10px;
    padding: 28px 36px 22px;
    margin-bottom: 20px;
    border-left: 5px solid #00A3E0;
    box-shadow: 0 6px 24px rgba(0,32,96,.22);
    position: relative; overflow: hidden;
">
  <div style="position:absolute;top:-30px;right:-30px;width:140px;height:140px;
              border-radius:50%;background:rgba(0,163,224,.08);pointer-events:none;"></div>
  <div style="display:flex;align-items:flex-start;gap:14px;margin-bottom:14px;">
    <div style="background:#00A3E0;border-radius:8px;width:42px;height:42px;
                display:flex;align-items:center;justify-content:center;font-size:22px;
                flex-shrink:0;box-shadow:0 2px 8px rgba(0,163,224,.35);">⚡</div>
    <div>
      <h1 style="color:#fff !important;font-size:1.7rem;font-weight:800;margin:0;
                 letter-spacing:-0.03em;-webkit-text-fill-color:#fff !important;
                 font-family:'Inter',sans-serif;">QA Test Cases Agent</h1>
      <p style="color:rgba(255,255,255,.70);margin:5px 0 0;font-size:13px;
                font-family:'Inter',sans-serif;">
        Intelligent multi-agent generation &nbsp;·&nbsp; Gemini &nbsp;·&nbsp; Groq &nbsp;·&nbsp; Ollama &nbsp;·&nbsp; LangGraph
      </p>
    </div>
  </div>
...
with st.sidebar:
    ...
    st.markdown(
        "<div class='qa-brand'>"
        "<div class='qa-brand-logo'>"
        "<svg width='22' height='22' viewBox='0 0 24 24' fill='none' xmlns='http://www.w3.org/2000/svg'>"
        "<path d='M13 2L4.09344 12.6879C3.74463 13.1064 3.57023 13.3157 3.56756 13.4925C3.56524 13.6461 3.63372 13.7923 3.75324 13.8889C3.89073 14 4.16316 14 4.70802 14H12L11 22L19.9065 11.3121C20.2553 10.8936 20.4298 10.6843 20.4324 10.5075C20.4348 10.3539 20.3663 10.2077 20.2468 10.1111C20.1093 10 19.8369 10 19.292 10H12L13 2Z'"
        " stroke='#00A3E0' stroke-width='2' stroke-linecap='round' stroke-linejoin='round'/>"
        "</svg>"
        "</div>"
        "<div>"
    ...
    st.markdown(
        "<div style='height:1px;background:#D0DCEE;margin:8px 0 10px;'></div>",
        unsafe_allow_html=True,
    )
    ...
    with st.expander("📚 Knowledge Base (RAG)", expanded=False):
        ...
        if not CHROMA_AVAILABLE:
            ...
            st.markdown(
                "<div style='background:#EBF0F8;border-left:3px solid #002060;"
                "border-radius:5px;padding:7px 10px;margin-bottom:8px;'>"
                "<span style='color:#002060;font-weight:700;font-size:13px;'>📋 Requirements</span>"
                "<div style='color:#5A6680;font-size:11px;margin-top:2px;'>"
            ...
            st.markdown(
                "<hr style='margin:10px 0;border-color:#D0DCEE;'>",
                unsafe_allow_html=True,
            )
            ...
            if CHROMA_AVAILABLE:
                ...
                st.markdown(
                    "<span style='color:#12253E;font-weight:700;font-size:13px;'>🔎 Test KB Retrieval</span>"
                    "<div style='color:#5A6680;font-size:11px;margin-bottom:6px;'>"
        ...
        with st.expander(
            ...
            for _hi, _he in enumerate(_history):
                ...
                _hc1.markdown(
                    ...
                    f"<span style='font-weight:700;color:#12253E;'>{_tag}</span>"
...
if not st.session_state.preview_rows and not run_clicked:
    ...
    _step3_bg     = "#EAFAF3" if _s3 == "✅" else "#EBF0F8"
    _step3_border = "#A3D9C0" if _s3 == "✅" else "#A8BEDD"
    _step3_color  = "#006644" if _s3 == "✅" else "#002060"
    _step3_sub    = (
    ...
    st.markdown(
        f"""
        <div style="background:#FFFFFF;border:1px solid #D0DCEE;border-radius:10px;
                    padding:28px 32px;margin:24px 0;box-shadow:0 2px 10px rgba(0,32,96,.07);
                    border-top:3px solid #002060;">
          <h2 style="color:#002060;margin:0 0 5px;font-size:1.25rem;font-weight:700;
                     font-family:'Inter',sans-serif;">Get started in 3 steps</h2>
          <p style="color:#5A6680;margin:0 0 22px;font-size:13.5px;font-family:'Inter',sans-serif;">
            Generate professional QA test cases from any spec document in minutes.
          </p>
          <div style="display:flex;gap:14px;flex-wrap:wrap;margin-bottom:24px;">
            <div style="flex:1;min-width:150px;background:#F5F8FC;border-radius:7px;
                        padding:14px;border:1px solid #D0DCEE;">
              <div style="font-size:1.4rem;margin-bottom:5px;">{_s1}</div>
              <div style="font-weight:700;color:#002060;font-size:12.5px;font-family:'Inter',sans-serif;">Select a model</div>
              <div style="color:#5A6680;font-size:11.5px;margin-top:3px;">
                Choose Gemini from the sidebar
              </div>
            </div>
            <div style="flex:1;min-width:150px;background:#F5F8FC;border-radius:7px;
                        padding:14px;border:1px solid #D0DCEE;">
              <div style="font-size:1.4rem;margin-bottom:5px;">{_s2}</div>
              <div style="font-weight:700;color:#002060;font-size:12.5px;font-family:'Inter',sans-serif;">Upload your spec</div>
              <div style="color:#5A6680;font-size:11.5px;margin-top:3px;">
                PDF, DOCX, TXT — or multiple files
              </div>
            </div>
            <div style="flex:1;min-width:150px;background:{_step3_bg};border-radius:7px;
                        padding:14px;border:1px solid {_step3_border};">
              <div style="font-size:1.4rem;margin-bottom:5px;">{_s3}</div>
              <div style="font-weight:700;color:{_step3_color};font-size:12.5px;font-family:'Inter',sans-serif;">Run the pipeline</div>
              <div style="color:#5A6680;font-size:11.5px;margin-top:3px;">
                {_step3_sub}
              </div>
            </div>
          ...
          <div style="display:grid;grid-template-columns:repeat(4,1fr);gap:10px;">
            <div style="text-align:center;padding:11px 8px;background:#EAFAF3;border-radius:6px;
                        border:1px solid #A3D9C0;">
              <div style="font-size:1.25rem;">🤖</div>
              <div style="font-size:10.5px;font-weight:600;color:#005C38;margin-top:4px;font-family:'Inter',sans-serif;">
                Multi-agent AI
              </div>
            </div>
            <div style="text-align:center;padding:11px 8px;background:#EBF0F8;border-radius:6px;
                        border:1px solid #A8BEDD;">
              <div style="font-size:1.25rem;">📊</div>
              <div style="font-size:10.5px;font-weight:600;color:#002060;margin-top:4px;font-family:'Inter',sans-serif;">
                Req coverage
              </div>
            </div>
            <div style="text-align:center;padding:11px 8px;background:#EBF5FB;border-radius:6px;
                        border:1px solid #7EC8E3;">
              <div style="font-size:1.25rem;">🥒</div>
              <div style="font-size:10.5px;font-weight:600;color:#003A6B;margin-top:4px;font-family:'Inter',sans-serif;">
                BDD / Gherkin
              </div>
            </div>
            <div style="text-align:center;padding:11px 8px;background:#FFF8EC;border-radius:6px;
                        border:1px solid #F9C97A;">
              <div style="font-size:1.25rem;">📚</div>
              <div style="font-size:10.5px;font-weight:600;color:#7A4800;margin-top:4px;font-family:'Inter',sans-serif;">
                RAG knowledge base
              </div>
            </div>
          </div>
...
if st.session_state.preview_rows:
        ...
        if not has_content:
            ...
            _conf_color = (
                ...
                else "#C53030" if isinstance(_avg_conf, int) else "#5A6680"
            )
            ...
            with _tab_tc:
                ...
                if not _display_rows:
                    st.markdown(
                        "<div style='text-align:center;padding:40px 20px;color:#5A6680;'>"
                    ...
                    if _tc_names:
                        with st.expander("🔎 View test case detail", expanded=False):
                            ...
                            _pri_c = {"Critical":"#C53030","High":"#D97706","Medium":"#002060","Low":"#5A6680"}.get(
                                _tc.get("Priority",""), "#5A6680")
                            st.markdown(
                                f"<div style='background:#F5F8FC;border:1px solid #D0DCEE;border-radius:8px;padding:16px 20px;'>"
                                f"<div style='font-size:15px;font-weight:700;color:#002060;margin-bottom:8px;'>"
                                ...
                                f"<span style='background:#EBF0F8;color:#002060;font-size:11px;padding:2px 8px;border-radius:4px;'>"
                ...
                if _is_full_strategy and st.session_state.strategy_bytes:
                    st.markdown(
                        "<div style='background:linear-gradient(135deg,#001040,#002060);"
                        ...
                        "<div style='color:rgba(255,255,255,.70);font-size:12px;font-family:Inter,sans-serif;'>"
            ...
            with _tab_analysis:
                ...
                _CAT_C = {"Functional":"#002060","Negative":"#C53030","Security":"#6B3FA0",
                           "Boundary":"#D97706","Performance":"#00875A"}
                _PRI_C = {"Critical":"#C53030","High":"#D97706","Medium":"#002060","Low":"#5A6680"}
                _LVL_C = {"Unit":"#8095B4","Integration":"#00A3E0","System":"#6B3FA0","Acceptance":"#00875A"}
            ...
            with _tab_cov:
                if not _cov or _cov.get("total_requirements", 0) == 0:
                    ...
                    st.markdown(
                        f"<div style='background:#E8EFF8;border-radius:4px;height:14px;"
                        ...
                        with st.expander(f"🟡 Partial coverage ({len(_partial)})"):
                            for _p in _partial:
                                ...
                                st.markdown(
                                    ...
                                    f" <span style='color:#5A6680;font-size:11px;'>[{_p.get('priority','')}]</span>"
                    ...
                    if _uncovered:
                        with st.expander(f"📋 All uncovered ({len(_uncovered)})"):
                            for _u in _uncovered:
                                _pc = {"Critical":"#C53030","High":"#D97706",
                                       "Medium":"#002060","Low":"#5A6680"}.get(
                                    _u.get("priority","Medium"), "#5A6680")
                                st.markdown(
                                    f"<div class='qa-kb-doc' style='margin:3px 0;'>"
                                    f"<span style='font-weight:600;color:{_pc};'>{_u['req_id']}</span>"
                                    f" <span style='color:#5A6680;font-size:11px;'>[{_u.get('priority','')}]</span>"
    ...
    if _meta_f:
        ...
        st.markdown(
            ...
            <div style="background:#f0fdf4;border-radius:10px;padding:10px 16px;
              ...
              &nbsp;·&nbsp;
              <span style="color:#5A6680;font-size:12px;">
