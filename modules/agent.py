...
from .config import TAB_SCHEMAS, get_llm, llm_invoke
...
# ─────────────────────────────────────────────────────────────
# Chunk result cache  (avoids re-running LLM on the same chunk)
# ─────────────────────────────────────────────────────────────

_CHUNK_CACHE_DIR = Path(__file__).resolve().parents[1] / "cache" / "chunks"
_CHUNK_CACHE_DIR.mkdir(parents=True, exist_ok=True)


def _chunk_cache_key(model_name: str | None, chunk: str, fields: list) -> str:
    """MD5 of (model, chunk text).  Fields are stable so we skip them from the hash."""
    raw = (model_name or "default") + chunk
    return hashlib.md5(raw.encode("utf-8")).hexdigest()


def _get_cached_chunk(key: str) -> list | None:
    path = _CHUNK_CACHE_DIR / f"{key}.json"
    if path.exists():
        try:
...
def _save_cached_chunk(key: str, rows: list) -> None:
    try:
        (_CHUNK_CACHE_DIR / f"{key}.json").write_text(
            _json.dumps(rows, ensure_ascii=False), encoding="utf-8"
        )
    except Exception:
        pass
...
def _semantic_dedup(cases: list, conf_scores: list) -> list:
    """
    Remove near-duplicate test cases using cosine similarity.

    Optimisation vs. v2: instead of making n individual ChromaDB queries
    (one per test case) we call the embedding function ONCE in batch mode
    and build the full similarity matrix with numpy.  This cuts dedup time
    from O(n × query_latency) → one embedding call + O(n²) CPU arithmetic,
    which is dramatically faster on CPU hardware.

    Falls back to name-based dedup if ChromaDB / numpy are unavailable.
    """
    if not cases:
        return cases
    ...
    try:
        ...
        # Single batch call — much cheaper than n individual ChromaDB round-trips
        embed_fn = embedding_functions.DefaultEmbeddingFunction()
        raw_embs  = embed_fn(docs)                           # list[list[float]]
        mat       = np.array(raw_embs, dtype=np.float32)    # (n, d)
        ...
        # cosine distance < 0.28  ≡  cosine similarity > 0.72
        _THRESH = 0.72
...
def document_parser_node(state: GraphState) -> dict:
    ...
    # For Ollama (CPU inference), skip the extra LLM pass and use the fast
    # heuristic parser.  API-backed models (Gemini, Groq) use the full
    # LLM-based parser which is much more accurate.
    is_ollama = isinstance(model_name, str) and model_name.startswith("ollama:")
    parsed_requirements: list = []

    if is_ollama:
        log.append("   🔍 Extracting requirements (heuristic — Ollama fast-path)...")
        try:
            from .req_parser import _heuristic_parse
            parsed_requirements = _heuristic_parse(text)
            log.append(f"   ✅ {len(parsed_requirements)} assertions identified (heuristic)")
        except Exception as e:
            log.append(f"   ⚠  Heuristic parsing failed: {str(e)[:60]}")
    else:
        log.append("   🔍 Extracting structured requirements (LLM)...")
        try:
...
def test_generator_node(state: GraphState) -> dict:
    ...
    ─ Parallel chunk processing (ThreadPoolExecutor).
      • API providers (Gemini / Groq) → up to 4 workers in parallel.
      • Ollama (CPU, serial queue) → 1 worker (no benefit from concurrency).
    ─ Per-chunk MD5 cache: identical chunks skip the LLM call entirely on re-runs.
    """
    log           = list(state.get("log", []))
    chunks        = state.get("chunks", [])
    collection    = state.get("collection")
    model_name    = state.get("model_name")
    prior_names   = list(state.get("generated_names", []))   # snapshot before this node
    ...
    is_ollama   = isinstance(model_name, str) and model_name.startswith("ollama:")
    max_workers = 1 if is_ollama else min(len(chunks), 4)
    mode_label  = "sequential (Ollama CPU)" if is_ollama else f"{max_workers} parallel workers"

    log.append(f"🤖 [Test Generator Agent] Processing {len(chunks)} chunk(s) — {mode_label}...")

    # ── ETA hint (based on historical timings) ─────────────────
    avg_sec = _timing.get_avg_seconds("generate_chunk") or 0.0
    if avg_sec > 0 and len(chunks) > 0:
        est_total = avg_sec * len(chunks) / max_workers
        log.append(f"   ⏱  Estimated generation time: ~{est_total:.0f}s")

    # ─────────────────────────────────────────────────────────────
    # Worker: process one chunk → returns (chunk_idx, rows, names, log_lines)
    # ─────────────────────────────────────────────────────────────
    def _process_chunk(args: tuple) -> tuple:
        ...
        try:
            # ── Cache check ────────────────────────────────────
            cache_key = _chunk_cache_key(model_name, chunk, fields)
            cached    = _get_cached_chunk(cache_key)
            if cached is not None:
                local_log.append(
                    f"   ⚡ Chunk {i + 1}/{len(chunks)} — cache hit ({len(cached)} cases)"
                )
                names = [str(r.get("Test Case Name", "")).strip() for r in cached
                         if r.get("Test Case Name")]
                return i, cached, names, local_log
            ...
            req_assertions = _find_relevant_reqs(chunk, parsed_reqs)

            local_log.append(f"   📝 Chunk {i + 1}/{len(chunks)}")
            ...
            # Give the LLM the names already generated BEFORE this parallel
            # batch so it can avoid obvious duplicates in its own output.
            if prior_names:
                system_prompt += (
            ...
            # ── LLM call ────────────────────────────────────────
            t0  = time.time()
            raw = llm_invoke(model_name, prompt, temperature=temperature)
            ...
            # Persist to cache for re-runs
            _save_cached_chunk(cache_key, non_empty)

            local_log.append(f"      ✅ {len(non_empty)} test case(s) extracted")
            names = [str(r.get("Test Case Name", "")).strip() for r in non_empty
                     if r.get("Test Case Name")]
            return i, non_empty, names, local_log

        except Exception as exc:
            local_log.append(f"      ❌ Chunk {i + 1} error: {str(exc)[:80]}")
            return i, [], [], local_log

    # ── Dispatch chunks ────────────────────────────────────────
    ordered_results: list[tuple | None] = [None] * len(chunks)

    with ThreadPoolExecutor(max_workers=max_workers) as ex:
        futures = {ex.submit(_process_chunk, (i, chunk)): i
        ...
        for fut in as_completed(futures):
            ...
            try:
                ...
                log.append(f"      ❌ Chunk {idx + 1} worker error: {str(exc)[:80]}")
                ordered_results[idx] = (idx, [], [], [])

    # ── Merge results in chunk order ───────────────────────────
    new_cases       = list(existing)
    generated_names = list(prior_names)

    for entry in ordered_results:
        if entry is None:
            continue
        _, cases, names, chunk_log = entry
...
def refiner_node(state: GraphState) -> dict:
    ...
    if weak_indices:
        ...
        for idx in weak_indices:
            ...
            try:
                improve_raw = llm_invoke(model_name, make_improve_prompt(tc, issues, fields), temperature=temperature)
    ...
    try:
        ...
        missing_raw = llm_invoke(model_name, gap_text, temperature=temperature)
        ...
        raw2 = llm_invoke(model_name, refine_prompt, temperature=temperature)
...
# ─────────────────────────────────────────────────────────────
# Build & compile  (compiled graph is a module-level singleton)
# ─────────────────────────────────────────────────────────────

_COMPILED_GRAPH = None   # cached after first build
...
def run_agent_graph(
    ...
    """
    # ── Ollama CPU: cap iterations to avoid excessive wait ────
    is_ollama = isinstance(model_name, str) and model_name.startswith("ollama:")
    if is_ollama:
...
def generate_strategy_content(
    ...
    try:
        ...
        raw = llm_invoke(model_name, prompt, temperature=0.1, max_retries=2)
